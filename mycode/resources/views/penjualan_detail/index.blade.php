...
<table id="tabel-penjualandet" class="table table-striped">
<thead>
   <tr>
      <th width="30">No</th>
      <th>Kode Produk</th>
  ...
  <div class="col-md-4">
    <form class="form form-horizontal form-penjualan" method="post" action="transaksi/simpan">
      ...
      <div class="form-group">
        ...
        <div class="col-md-8">
          <div class="input-group">
            <input id="nama_member" type="text" class="form-control" name="nama_member" value="Umum">

            <input id="member" type="hidden" class="form-control" name="member" value="umum" readonly>
            <span class="input-group-btn">
...
$(function(){
  ...
  table = $('#tabel-penjualandet').DataTable({
    ...
    loadForm($('#diskon').val($('.diskon').text()));
  });
...
function selectMember(kode,namamember,diskon){
  loadForm($('#diskon').val());
  ...
  $('#diterima').val(0).focus().select();
      ...
      //var diskon = '{{ $setting->diskon_member }}';
      var id = $('#idpenjualan').val();
      //alert(diskon+"-"+id);
      $.ajax({
          url : "transaksi/diskon/"+id+"/"+diskon+"/"+kode,
          type : "GET",
      });
}
